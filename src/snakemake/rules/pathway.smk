...
rule ldsc_partitioned_h2:
    ...
    the condition-number error (see download_ldsc_afr_frq docstring NOTE
    + .planning/debug session §2026-04-18T13:00Z "Bug 5").

    Condition-number bypass (Launch12 Bug 5): the joint baselineLD (97
    annotations) + custom_pathway matrix is intrinsically near-singular
    (cond > 1e5 for all ancestries). ``run_partitioned_h2`` passes
    ``--invert-anyway`` per LDSC FAQ — the canonical S-LDSC remedy for
    multi-annotation models including the full baselineLD set.
    """
    input:
        munged=os.path.join(PATHWAY_RESULTS_DIR, "ldsc_partitioned", "munged", "{trait}_{ancestry}.sumstats.gz"),
        custom_ld=expand(
