...
rule build_1kg_sample_lists:
    ...
    shell:
        r"""
        {PYTHON_BIN} scripts/build_1kg_sample_lists.py \
            --panel {input.panel} \
...
rule build_ld_rds:
    ...
    params:
        ...
        rscript=config["finemap"]["rscript_bin"]
    conda:
        "../../envs/snakemake_env.yml"
    threads: 1
    resources:
    ...
    shell:
        r"""
        set -euo pipefail
        {PYTHON_BIN} scripts/build_ld_rds.py \
            --vcf {input.vcf} \
...
rule collect_region_variants:
    ...
    shell:
        ...
        {PYTHON_BIN} scripts/collect_region_variants.py \
            --region-id {params.region_id} \
