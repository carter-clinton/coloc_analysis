...
(1) Real-LD re-analysis at curated cardiometabolic disease loci is rare in the published coloc literature; the identity-LD vs real-LD head-to-head comparison at the same loci with the same pipeline is, to our knowledge, the first systematic audit of its kind at this trait set. (2) The Snakemake-pinned pipeline with conda-environment specifications enables exact reproduction by any reviewer. (3) Pre-registered negative-control loci (ABO blood-group, cosmetic pigmentation, HLA) behave as expected — all 224 negative-control rows resolved to Tier C or empty, zero reached Tier A/B — confirming pipeline calibration. (4) The 1,242 QTL-coloc failures and 28/28 trait-pair empty-PP coloc.susie outputs are diagnosed honestly against specific commit-level pipeline edge cases rather than reinterpreted as biology.
