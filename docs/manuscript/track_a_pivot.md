...
**SH2B3 12q24, anchor example.** Under the Stage 1d identity-LD pass, SuSiE-RSS + `coloc.susie` at *SH2B3* (12q24, EUR) produced PP.H4 = 1.00 for the BMI–hypertension and hypertension–stroke trait pairs at canonical leads (rs3184504, rs10774625, rs7137828, rs4766578), matching the canonical single-causal-variant `coloc.abf` claim in the prior literature. Under the Stage 2 real-LD re-fit (1000G Phase 3 EUR, commits `6de9a88` + `a6e3214` + `7d54183`), the Phase 1 SuSiE-RSS fits at *SH2B3* EUR for BMI, hypertension, and stroke were re-computed with the real LD matrix in place of the identity-fallback LD — and the Stage 2 `coloc.susie` output at the SH2B3_12q24 EUR region contains only the asthma-vs-T2D trait-pair row (`SH2B3_12q24__EUR__asthma_vs_t2d`), with empty PP.H3/PP.H4/n_snps columns. The canonical BMI–hypertension and hypertension–stroke trait pairs at SH2B3 EUR are **absent from the Stage 2 `coloc.susie` output manifest**, consistent with credible-set collapse on at least one partner trait under real-LD (the test requires non-empty credible sets on both sides). The QTL-coloc side at SH2B3_12q24 EUR resolves only to Tier C at PP.H4 = 0.0517 (*ATXN2* / Adrenal_Gland / GTEx eQTL) — far below any plausible survival of the published PP.H4 = 1.00 claim. This is the most dramatic flagship change in this dataset and illustrates the inflation mechanism: under identity-LD, SuSiE-RSS resolves the trait-A signal into an inflated credible set, and the colocalization test treats correlated marginal peaks as shared causality; under real-LD, SuSiE-RSS correctly fails to produce a credible set (or produces one so small that `coloc.susie` returns no-signal). **Scope caveat**: a targeted re-fire of `coloc.susie` for the canonical SH2B3 trait-pairs under Stage 2 real-LD (rather than the current asthma-vs-T2D restriction) is pre-registered as a supplementary re-analysis to fully close out the flagship claim; see Supplementary Methods §Post-freeze execution roadmap.
...
|---|---|---|---|---|---|---|---|---|
| SH2B3_12q24 | SH2B3 | BMI–hypertension | 1.00 (Stage 1d) | absent from Stage 2 output | — | — | 0 (inferred) | **lost (pair absent from Stage 2 manifest — re-fire pre-registered)** |
| SH2B3_12q24 | SH2B3 | hypertension–stroke | 1.00 (Stage 1d) | absent from Stage 2 output | — | — | 0 (inferred) | **lost (pair absent from Stage 2 manifest — re-fire pre-registered)** |
