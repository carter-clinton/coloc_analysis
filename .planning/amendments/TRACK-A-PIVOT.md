...
(Abstract omits "machine learning" entirely, demotes evolutionary medicine, names the SH2B3 collapse and the 51/96 vs 12/96 CS yield contrast, and reframes the 1,005 too_few_snps + 28 trait-pair failure modes honestly per disk-truth.)
...
- **Rewrite.** Replace the current single paragraph with:
  ...
  (c) Output summary: **51/96 non-empty credible sets under real-LD at admissible regions, vs 12/96 under identity-LD fallback** (source: `results/fine_mapping/finemap_summary.tsv`, confirm exact denominator at freeze).
...
- **New figure.**
- **Content**: paired histogram or beeswarm. For each of the 96 admissible SuSiE fits, plot credible-set size under identity-LD vs real-LD. Show the 51/96 non-empty vs 12/96 contrast.
...
4. **Fine-mapping credible-set yield**: 51/96 vs 12/96 non-empty CS. Source: `results/fine_mapping/finemap_summary.tsv` and `finemap_summary_augmented.tsv`. Re-confirm denominators at freeze. Feeds Abstract, Methods 4.10, Results 4.16 overview, Figure 2.
...
- Re-compute item 3 (SH2B3 specific numbers) and item 4 (51/96 vs 12/96) to lock abstract numbers.
