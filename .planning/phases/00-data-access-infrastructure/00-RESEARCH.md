...
1. **TCF7L2 region coordinates and legacy PP.H4 values** (RESOLVED)
   ...
   - **RESOLVED:** Region coordinates are chr10:114550000-115150000 (+-500kb around rs7903146). Used in Plan 04 toy locus definition (regions_toy.csv). Legacy PP.H4 = 1.000 from manuscript; placeholder expected values in expected_results.yaml will be updated after first real run.

2. **R package availability on conda-forge/bioconda** (RESOLVED)
   ...
   - **RESOLVED:** r-coloc is assumed available on conda-forge/bioconda per Plan 01 (legacy env listed them as conda deps). Plan 01 Task 2 includes a fallback note: if not available as conda packages, use r-remotes with a post-deploy install script.

3. **Genome-wide manifest TSV handling** (RESOLVED)
   ...
   - **RESOLVED:** The manifest TSV is treated as regenerated by build_run_manifest.py (or equivalent). Plan 03 parameterizes the generator script; if the script does not exist, the manifest will be regenerated from config/pipeline.yaml during Phase 0 execution.
