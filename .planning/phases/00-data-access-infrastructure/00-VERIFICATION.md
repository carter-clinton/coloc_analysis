...
| 7 | Toy 3-locus CI smoke test completes in under 15 minutes | PARTIAL (scaffolding verified 2026-04-10T23:14:00Z) | Scaffolding complete AND dry-run verified: `snakemake -n --snakefile tests/toy_3locus/Snakefile.test` resolves a valid DAG with 29 jobs across 11 rules (download_sumstats × 3, harmonize_sumstats × 3, download_1kg_panel, download_1kg_vcf × 3, build_1kg_sample_lists, make_regions_from_loci, collect_region_variants × 3, build_ld_rds × 3, build_finemap_manifest, run_finemap × 9, all). 5 scaffolding bugs found and fixed during dry-run: (1) validate() schema path relative-to-Snakefile issue; (2-4) 3 × include: path same issue (fixed to `../../src/...`); (5) FINEMAP_OUTPUTS definition order (moved before finemap.smk include); plus FINEMAP_SUMMARY added to match production Snakefile. 1 critical environment constraint documented: Snakemake 7.32.4 is incompatible with Python 3.12+ f-string tokenization (PEP 701), smoke test requires Python 3.11. Dev env created at `/rs1/researchers/c/ckclinto/conda_envs/smoke_dev/` for dry-run verification. First real execution (with data downloads + conda env creation + 15-minute runtime check) deferred to Phase 1 first execution — scaffolding is now proven sound. |
