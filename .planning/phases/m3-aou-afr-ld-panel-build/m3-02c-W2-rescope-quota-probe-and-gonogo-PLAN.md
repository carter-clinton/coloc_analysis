...
CARRY-FORWARD FROM m3-02b (BR-01 blast-radius sweep, 2026-06-19 — MUST be addressed fully before the A.3 LD panel is shipped as "LD + AF"):
**Path A.3 regions currently get NO allele frequency.** `src/python/bm_to_npz.py` (the A.3 BlockMatrix->.npz converter) writes only `ld` + `variant_ids` + `rsids` (+ now the `lower_triangular` flag from the BR-01 fix) — it writes NO `allele_freq` key, so `ld_npz_to_rds.R` fills `obj$variants$AF` with NA for every A.3 (large/xlarge) region. The phase deliverable is "LD + AF metadata", and A.3 IS the large/xlarge regions, so this is a real coverage hole — pre-existing, NOT a CR-01-fix regression, deferred out of the m3-02b code plan. **The probe in this plan fires A.3 sub-regions (m2_region_00040__sub00, region_00145) — so the AF gap surfaces here; treat closing it as a precondition for the m3-04 production A.3 fire.** Full fix (NCSU/in-perimeter code, its own task — do NOT hand-wave): (1) the A.3 branch in `aou_ld_panel.py::compute_region_ld` must EXPORT an AF sidecar (row-aligned to the variant_ids/rsids sidecars it already writes for the BlockMatrix) using the same `_af_or_nan` collection as A.1/A.2; (2) `bm_to_npz.py` must READ that AF sidecar and write `allele_freq=` into the `.npz` (NaN for genuinely-missing, per WR-03); (3) add a round-trip test that an A.3-style `.npz` carries row-aligned AF into `obj$variants$AF`. Detail + rationale: `m3-02b-W2-rescope-BLAST-RADIUS-FIX.md` (DEFERRED section).
</objective>
