...
### D-M3-10: MT write verification protocol — contents-validated, not _SUCCESS-only

**Decision:** Every `mt.checkpoint()` write touchpoint in M3 (Wave 1 cohort MTs, Wave 2 dev-fire LD intermediates, Wave 4 production LD outputs, Wave 5 closeout verification) MUST go through contents-validating verification — `_validate_checkpoint_populated()` (Python) + entries/entries/parts/ size assertion (gsutil + Hail read-probe). The `_SUCCESS` marker alone is **NEVER** sufficient evidence of populated data. No new code-side `_has_checkpoint(ckpt_*)` callers may be introduced for resume-gate semantics; the only acceptable use of `_has_checkpoint()` is as a fast pre-filter inside `_validate_checkpoint_populated()` itself, or as the explicit catastrophe-pattern guard in `load_qc_cohort`. (Decision locked 2026-05-28 by Track 4 defensive-code patches; quick task 260528-jvd.)

**Why:** The m3-W1 empty-MT catastrophe (2026-05-18→05-21; ~$2,100 burned) proved that Hail's `mt.checkpoint()` writes `_SUCCESS` based on driver-side tasks-reported-complete accounting WITHOUT validating output contents. Under aggressive `spark.executor.cores=1/mem=5g` profile (necessary for v8 partition-explosion OOM remediation per DEC-2026-05-04-01), executor tasks can silently truncate after writing 35-byte Parquet column-metadata footer stubs, leaving an MT directory with `_SUCCESS` + rows-stubs + absent `entries/entries/parts/` + `count_rows()=0 + count_cols()=0` on read-back. Three pre-2026-05-21 verification artifacts (`_SUCCESS` listing alone; Spark UI stack-trace cascade-signature inference; in-kernel `count_rows()` calls satisfied from JVM-cached IR) ALL falsely declared the catastrophe MTs "OK". The bucket forensics 2026-05-21 found mt_afr_qc.mt and mt_afr_pca_selfid_qc.mt as 0×0 stub MTs and mt_eur_qc.mt directory absent altogether. Without this decision the refactored `load_qc_cohort` (commit chain ending bd144a6, quick task 260518-qcr) would happily resume from these stubs and reproduce the catastrophe under credit-recovered or grant-funded rebuild — the resume-gate was the failure mode the refactor did not defend against. Per `[[feedback_rigor_over_speed]]`: the ~1-2 min total overhead per cohort write for `mt.count_rows() + mt.count_cols()` post-write actions is trivial against the 60+ h monolithic-run baseline and worth every second of compute it costs.

**How to apply:** (1) `src/python/aou_ld_panel.py` — `_validate_checkpoint_populated()` helper (added m3-W1 Track 4 patch 2/7) is the canonical resume-gate; auto-resume state machine calls it at every `_has_checkpoint(ckpt_*)` site that decides "is this MT real?" (patch 3/7); every `mt.checkpoint(uri, ...)` call has a post-write `_assert_checkpoint_nonempty(mt, uri, phase=...)` that raises RuntimeError on `count_rows()==0 OR count_cols()==0` (patch 4/7). (2) `.planning/notebooks/AOU-1_template.ipynb` — Cells 3.5 / 4.5 / 5.5 fire `gsutil du -s` against `entries/entries/parts/` and assert size > 1 GB before the next cell defines a downstream cohort on top (patch 5/7). (3) `.planning/WAVE-1-CLOSEOUT-CHECKLIST.md` STEP 3 — verifies entries size + adds belt-and-suspenders Hail-side read-probe from a fresh Python subprocess (NOT the same kernel that did the write — JVM-cached IR can mask the catastrophe), per patch 6/7. (4) Any future M3 notebook (AOU-2 per-region LD compute, AOU-4 validation memo, dev-10 smoke-test, chr22 smoke pre-fire) MUST inherit the same pattern: gsutil + Hail read-probe + size assertion per MT write target. (5) `_SUCCESS` markers may still be read for FAST-PATH "is anything there at all?" probes, but the resume-gate decision is gated on the contents-validating check. (6) chr22 smoke fires need a sub-1GB threshold cell variant — document the override path in the smoke notebook. Cross-references: `.planning/debug/m3-W1-empty-mt-catastrophe.md` (root cause), memories `[[feedback_aou_success_marker_not_evidence_of_data]]` + `[[feedback_hail_checkpoint_contract_violation]]` + `[[feedback_aou_spark_ui_stack_trace_verification]]` (amended). Track 4 patch list: `.planning/quick/260521-w1-catastrophe-handoff/HANDOFF.md` §Track 4.
...
10. **Every MT write touchpoint is contents-validated, not _SUCCESS-only.** D-M3-10 — locked 2026-05-28 by Track 4 defensive-code patches (quick task 260528-jvd) in response to the m3-W1 empty-MT catastrophe (~$2,100 lost producing 0×0 stub MTs that passed pre-patch verification). Resume-gate semantics MUST use `_validate_checkpoint_populated()` (verifies `_SUCCESS` + `entries/entries/parts/` size + Hail count_rows>0). Every `mt.checkpoint()` site MUST have a post-write `_assert_checkpoint_nonempty()` that raises on empty contents. Notebook layer MUST gsutil-du-verify entries-dir size before downstream cells fire. Closeout layer MUST cross-check via Hail read-probe from a fresh Python subprocess (NOT the same kernel that did the write — JVM-cached IR can mask the catastrophe). `_SUCCESS` is NEVER sufficient evidence of populated data per `[[feedback_aou_success_marker_not_evidence_of_data]]` + `[[feedback_hail_checkpoint_contract_violation]]`.
</assumptions>
