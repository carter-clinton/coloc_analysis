...
| **C13** Honest-framing-lock anchors preserved (content-based per RESEARCH.md Pitfall 7) | 6 | REQ-OSF-PREREG | unit | Pre-rename + post-narrative captures via `grep -nE` on the 4 content phrases (`^### .*SH2B3.*case[- ]study`, `SUPERSEDED`, `Identity-LD Inflation`, `Harmonization-Pipeline Diagnostics`) into `honest_framing_anchors_pre.txt` + `honest_framing_anchors_post.txt`; verify via `diff <(grep -v '^[0-9]*:' pre \| sort) <(grep -v '^[0-9]*:' post \| sort)` | strip line-number prefix; content byte-identical between pre and post (line numbers may shift) | ⬜ pending |
