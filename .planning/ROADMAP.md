...
**Plans**: 8 plans (m3-00 through m3-05 + m3-02b/m3-02c Wave-2 re-scope; 8 waves)
...
- [ ] m3-02b-W2-rescope-split-stitch-code-PLAN.md — Wave 0 (re-scope; autonomous NCSU code): SPLIT the ~16 xlarge regions at manifest-build time into __sub{k:02d} rows (--max-subregion-span-mb default 10 + --split-classes xlarge; parent_region_id/subregion_index/n_subregions/subregion_start/end_grch38 provenance + split_status projection; radius re-derived on sub-span so NO sub-region is A.3) + dev-subset expansion of a selected xlarge parent + NEW stitch_subregions_to_rds.R (N sub-region .npz -> 1 parent .rds via sparse Matrix::bdiag block-diagonal, zeroed cross-block per WAVE-2 HIGH-3, position-sorted dimnames, PSD per-block) + A6 finemap sparse-payload verify + AOU-2 Q-RS2 executor cell (cores=2/24g/10g/24g) + all 9 named test families. Supersedes the dev-fire compute step of m3-02 (which was killed as intractable). LOCKED: A.3 fix correct; ordering A kept; cohorts intact.
- [ ] m3-02c-W2-rescope-quota-probe-and-gonogo-PLAN.md — Wave 1 (re-scope; autonomous:false, Carter fires in-perimeter): N2_CPUS>=400 us-central1 quota human-action gate (LONGEST-LEAD, surfaced FIRST) + the 2-3 cell real-cohort cost probe on the sized n2-highmem-16 cluster (WHOLE region_00006 + xlarge SUB-REGION m2_region_00040__sub00 [+ optional EUR], blocks_per_min into m3-W2-cost-probe.tsv, data-layer-verified per D-M3-10) + redo_ld_cost_model.py re-derives the 322-cell budget + the EXACT go/no-go gate PROJECTED x 1.3 <= BUDGET_CAP_CLUSTER_H (GREEN/YELLOW-narrow-radius/YELLOW-finer-split/RED). Ends at a defensible go/no-go decision; the full 322-cell production fire stays OUT of scope (Wave 4 / m3-04).
