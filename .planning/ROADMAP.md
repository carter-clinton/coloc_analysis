...
**Plans**: 16 plans (m3-00 through m3-05 + m3-02b/m3-02c/m3-02d/m3-02e Wave-2 re-scope + **m3-04b/m3-04c Wave-4 replan** + m3-06 + m3-07a/m3-07b/m3-07c post-loop NaN→occlusion waves; 14 waves)
...
4. **Conditioned artifact** — write conditioned `.npz`/`.rds` with provenance keys
   ...
   ⚠ **CORRECTED 2026-08-12** (`quick-260812-09a`, claim-level sweep): **`ld_npz_to_rds.R` is
   NOT frozen and was never frozen.** It measures **+313/−62** against the `bf16289` the record
   claimed pinned it, and `DEC-2026-08-11-sr4-disposition` answered SR4-OPEN as **NEVER ACTUALLY
   FROZEN** for all five files. The genuinely gated files are the three named by
   `tests/m3/test_source_freeze_pins.py`. What this plan step actually means is that the raw
   `.npz` **format contract** is not to be changed by the conditioning work — an intent about
   the artifact, not a byte pin on the reader. The `.npz` contract statement stands; the
   parenthetical about the file does not.
