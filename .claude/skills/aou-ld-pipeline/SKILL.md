...
> - **The current gate surface is m3-04c Task 3 plus `.planning/quick/260811-rcw-…/260811-rcw-PRE-FIRE-GATE-REVIEW.md`** (corrected 2026-08-12 — read its `## Corrections (2026-08-12)` section first; its liveness-poll command was double-prefixed and would have FALSE-PASSED its own pre-fire check).
...
- 🔴 **GATE 3** — ⚠ **RETIRED PRODUCER (banner 2026-08-12): this row describes the killed Hail A.3 path and is NOT a live blocker.** The current equivalent is m3-04c Task 3 + the corrected 260811-rcw PRE-FIRE gate review (native plink, AFR-only, 276 regions, ≤22 egress groups). full 322-cell production + 44 egress. **BLOCKED on CR-01** (the A.3 ~2 TB dense-scratch ordering question) + land the durable atomic-final-write fix first.
